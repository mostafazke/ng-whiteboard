...
export class ArrowBindingService implements OnDestroy {
  ...
  attachStart(arrow: ArrowElement, elementId: string, pointId: string | null, gap = 0): Partial<ArrowElement> {
    ...
    const otherEnd = { x: arrow.x2, y: arrow.y2 };
    const point = this.resolveEndpoint(binding, { x: arrow.x1, y: arrow.y1 }, otherEnd);
    return {
      id: arrow.id,
  ...
  attachEnd(arrow: ArrowElement, elementId: string, pointId: string | null, gap = 0): Partial<ArrowElement> {
    const binding = this.createBinding(elementId, pointId, gap);
    const otherEnd = { x: arrow.x1, y: arrow.y1 };
    const point = this.resolveEndpoint(binding, { x: arrow.x2, y: arrow.y2 }, otherEnd);
    return {
      id: arrow.id,
      endBinding: binding,
      x2: point.x,
  ...
  recomputeBindingsForElements(movedIds: Set<string>): Array<Partial<ArrowElement> & { id: string }> {
    ...
    for (const arrow of arrows) {
      ...
      if (arrow.startBinding && movedIds.has(arrow.startBinding.elementId)) {
        const otherEnd: Point = { x: arrow.x2, y: arrow.y2 };
        const newStart = this.resolveEndpoint(arrow.startBinding, { x: arrow.x1, y: arrow.y1 }, otherEnd);
        update.x1 = newStart.x;
        update.y1 = newStart.y;
        changed = true;
      }
      ...
      if (arrow.endBinding && movedIds.has(arrow.endBinding.elementId)) {
        const otherEnd: Point = {
          x: update.x1 !== undefined ? update.x1 : arrow.x1,
          y: update.y1 !== undefined ? update.y1 : arrow.y1,
        };
        const newEnd = this.resolveEndpoint(arrow.endBinding, { x: arrow.x2, y: arrow.y2 }, otherEnd);
        update.x2 = newEnd.x;
        update.y2 = newEnd.y;
        changed = true;
      }
