...
import { ArrowHeadStyle, ElementType, Point, PointerInfo, SnapResult, ToolType, WhiteboardElementStyle } from '../types';
...
export class ArrowTool extends BaseTool {
  ...
  override handlePointerMove(event: PointerInfo): void {
    ...
    if (Math.abs(x2 - this.lastX) > this.MIN_LENGTH || Math.abs(y2 - this.lastY) > this.MIN_LENGTH) {
      const draftUpdate: Partial<ArrowElement> = { id: this.element.id, x2, y2 } as Partial<ArrowElement> & { id: string };
  ...
  private mapHeadStyle(style?: ArrowHeadStyle): ArrowheadType {
    ...
    switch (style) {
      case 'none':        return ArrowheadType.None;
      case 'arrow':       return ArrowheadType.Arrow;
      case 'open-arrow':  return ArrowheadType.OpenArrow;
      case 'diamond':     return ArrowheadType.Diamond;
      case 'open-diamond':return ArrowheadType.OpenDiamond;
      case 'circle':      return ArrowheadType.Circle;
      case 'open-circle': return ArrowheadType.OpenCircle;
      case 'bar':         return ArrowheadType.Bar;
      default:            return ArrowheadType.None;
    }
  }
}
